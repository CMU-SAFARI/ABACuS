...
namespace Ramulator
{
  ...
  template <typename T>
  void RowTable<T>::update(typename T::Command cmd, const vector<int>& addr_vec, uint64_t clk, int core_id)
  {
    ...
    if (spec->is_opening(cmd))
        table.insert({rowgroup, {row, 0, clk, clk, core_id}});
  ...
  template <typename T>
  int RowTable<T>::get_core_id(const vector<int>& addr_vec) 
  {
    auto begin = addr_vec.begin();
    auto end = begin + int(T::Level::Row);
