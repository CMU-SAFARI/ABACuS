...
namespace Ramulator
{
  ...
  class Controller
  {
    ...
    public:
      ...
      ActInjector<T>* act_injector = nullptr;     ///< Inject ACTs to simulate RowHammer
    ...
    public:
      ...
      /**
       * @brief    Sends a request to the otherq in the memory controller.
       * 
       * @param    req            The request.
       * @return   true           Return true if successful.
       * @return   false          Return false if the corresponding request queue is full.
       */
      virtual bool priority_enqueue(Request& req);
    ...
    protected:
       ...
       */
      virtual void issue_cmd(typename T::Command cmd, const vector<int>& addr_vec, int core_id, uint64_t unique_id);
    ...
    protected:
      ...
      void print_trace(typename T::Command cmd, const vector<int>& addr_vec, uint64_t unique_id);
