...
namespace Ramulator
{
  ...
  cmd_trace_files(channel->children.size())
  {
    ...
    YAML::Node config_no_const = YAML::Clone(config);
    rh_defense  = make_refresh_based_defense<T> (config_no_const, this);

    if (config["act_injector"]) {
      act_injector = new ActInjector<T> (config["act_injector"], this);
    }
    ...
    num_cores = 1; // until overwritten by reg_stats();
  ...
  void Controller<T>::reload_options(const YAML::Node& config)
  {
    ...
    config_no_const["controller"]["num_cores"] = num_cores;

    if (rh_defense == nullptr)
      rh_defense  = make_refresh_based_defense<T> (config_no_const["controller"], this);

    if (act_injector != nullptr) {
      act_injector->enable();
    }

    channel->spec->reload_options(config);
  ...
  void Controller<T>::finish(long read_req, long dram_cycles)
  {
    ...
    channel->finish(dram_cycles, clk, &total_energy);

    total_dram_energy = total_energy;
  }
  ...
  bool Controller<T>::enqueue(Request& req)
  {
    ...
    // preventive refreshes can tolerate latency but periodic refreshes cannot
    if (req.type == Request::Type::PREVENTIVE_REFRESH)
      if (queue.size() >= (queue.max - 5))
        return false;
  ...
  template <typename T>
  bool Controller<T>::priority_enqueue(Request& req)
  {
    ReqQueue& queue = otherq;
    if (queue.max == queue.size())
      return false;

    // print that a request is being enqueued
    // printf("Request enqueued at time %lu\n", clk);
  ...
  bool Controller<T>::find_request_to_schedule(ReqQueue*& queue, std::list<Request>::iterator& req, typename T::Command& cmd, std::vector<int>& addr_vec)
  {
    ...
    if (!is_req_valid) 
    {
      ...
      // if there is a valid&ready request in these queues
      // check if the otherq has a request that targets the same rowgroup (bank)
      // if so, do serve the request from the otherq first
      ...
      if (is_req_valid && otherq.size())
      {
        ...
        if (find_if(otherq.q.begin(), otherq.q.end(),
              [rowgroup](Request& req){ return rowgroup == std::vector<int>(req.addr_vec.begin(), req.addr_vec.begin() + int(T::Level::Row));}) != otherq.q.end())
          is_req_valid = false;
      }

      // finally check if any of the requests in the otherq is a periodic refresh
      if (is_req_valid)
      {
        auto begin = _req->addr_vec.begin();
        auto end = begin + int(T::Level::Row);
        ...
        // if there is a refresh request, make is_req_valid false
        if (find_if(otherq.q.begin(), otherq.q.end(),
              [rowgroup](Request& req){ return req.type == Request::Type::REFRESH;}) != otherq.q.end())
          is_req_valid = false;
      }
      ...
      if(is_req_valid)
      {
        ...
        if (is_req_valid && channel->spec->is_closing(_cmd))
        {
          auto begin = _req->addr_vec.begin();
          auto end = begin + int(T::Level::Row);
          auto end2 = begin + int(T::Level::BankGroup);
          std::vector<int> rowgroup(begin, end);
          std::vector<int> rowgroup2(begin, end2);
          ...
          // The above check is not sufficient for preAB who has its bg and bank addresses set to -1
          if (is_req_valid && _cmd == T::Command::PREab)
          {
            for (auto it = actq.q.begin(); it != actq.q.end(); it++)
            {
              std::vector<int> it_rowgroup(it->addr_vec.begin(), it->addr_vec.begin() + int(T::Level::BankGroup));
              if (it_rowgroup == rowgroup2)
              {
                is_req_valid = false;
                break;
              }
    ...
    {
      // if request type is refresh, print it
      // if (_req->type == Request::Type::REFRESH && (otherq.size() > 25))
      //   std::cout << "Refresh request issued" << std::endl;
      queue = _queue;
      req = _req;
      cmd = _cmd;
  ...
  void Controller<T>::serve_pending_AP()
  {
    for(auto addr_vec_itr = pending_autoprecharges.begin(); addr_vec_itr != pending_autoprecharges.end(); )
    {
      ...
      {
        issue_cmd(T::Command::PRE, *addr_vec_itr, rowtable->get_core_id(*addr_vec_itr), 0);
  ...
  void Controller<T>::tick()
  {
    ...
    if (found_cmd)
    {
      ...
      issue_cmd(cmd, addr_vec, req->coreid, req->unique_id);
  ...
  template <typename T>
  void Controller<T>::issue_cmd(typename T::Command cmd, const vector<int>& addr_vec, int core_id, uint64_t unique_id)
  {
    ...
    if (rh_defense != nullptr)
    {
      ...
      int _core_id = rowtable->get_core_id(precharged_address_vec);
      rh_defense->update(cmd, precharged_address_vec, open_for_nclocks, _core_id);
    }

    if (track_activation_count && cmd == T::Command::ACT)
      process_activate(addr_vec);

    assert((!channel->spec->is_opening(cmd)) || (addr_vec[int(T::Level::Row)] >= 0));

    rowtable->update(cmd, addr_vec, clk, core_id);

    if (record_cmd_trace)
      dump_trace(cmd, addr_vec);

    if (print_cmd_trace)
      print_trace(cmd, addr_vec, unique_id);
  }
  ...
  template <typename T>
  void Controller<T>::print_trace(typename T::Command cmd, const vector<int>& addr_vec, uint64_t unique_id)
  {
    printf("%10ld %5s %10ld:", unique_id, channel->spec->command_str[int(cmd)].c_str(), clk);
  ...
  {
    // perhaps there is a better way to communicate this information
    // to the rowhammer defense
    ...
    total_dram_energy
        .name("total_dram_energy"+to_string(channel->id) + "_channel")
        .desc("DRAM energy consumption per channel")
        .precision(12)
        ; 
    ...
    preventive_refresh_bank
        .init(channel->spec->org_entry.count[int(T::Level::Rank)]*channel->spec->org_entry.count[int(T::Level::BankGroup)]*channel->spec->org_entry.count[int(T::Level::Bank)])
        .name("preventive_refreshes_channel_"+to_string(channel->id) + "_bank_")
        .desc("Number of preventive refreshes per bank")
        .precision(0)
        ;    
  ...
  void Controller<T>::update_request_stats(std::list<Request>::iterator& req)
  {
    ...
    {
      ++preventive_refreshes;
      int flat_bank = req->addr_vec[int(T::Level::Bank)] + 
      req->addr_vec[int(T::Level::BankGroup)] * channel->spec->org_entry.count[int(T::Level::Bank)] + 
      req->addr_vec[int(T::Level::Rank)] * channel->spec->org_entry.count[int(T::Level::BankGroup)] * channel->spec->org_entry.count[int(T::Level::Bank)] ;
      ++preventive_refresh_bank[flat_bank];
    }
