...
namespace Ramulator
{
  ...
  TranslationBase::TranslationBase(const YAML::Node& config, uint64_t max_address)
  {
    ...
    this->max_address_pages = max_address >> 12;
  ...
  uint64_t LessRandomTranslation::page_allocator(uint64_t addr, int coreid)
  {
    ...
    if(page_translation.find(target) == page_translation.end()) 
    {
      // page doesn't exist, so clear all mappings
      if (free_physical_pages_remaining == 0)
      {
        assert(current_contiguously_allocated_pages == contiguous_allocation_quota);
        // clear all mappings
        for (size_t i = 0 ; i < free_physical_pages.size() ; i++)
          free_physical_pages[i] = -1;
        free_physical_pages_remaining = free_physical_pages.size();
      }

      if (current_contiguously_allocated_pages == contiguous_allocation_quota)
      {
        uint64_t random_number = (allocator_rng() << 48) ^ (allocator_rng() << 32) ^ (allocator_rng() << 16) ^ allocator_rng();
        // assign a new page
        uint64_t phys_page_to_read = (random_number % (free_physical_pages.size() / contiguous_allocation_quota)) * contiguous_allocation_quota;

        // if page is blacklisted, retry
        while (std::find(blacklist.begin(), blacklist.end(), phys_page_to_read) != blacklist.end())
        {
          random_number = (allocator_rng() << 48) ^ (allocator_rng() << 32) ^ (allocator_rng() << 16) ^ allocator_rng();
          phys_page_to_read = (random_number % (free_physical_pages.size() / contiguous_allocation_quota)) * contiguous_allocation_quota;
        }
    ...
    // print
    // std::cout << std::hex << ((page_translation[target] << 12) | (addr & ((1 << 12) - 1))) << std::dec << std::endl;
