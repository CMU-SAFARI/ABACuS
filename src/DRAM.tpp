...
{
  template <typename T>
  void DRAM<T>::finish(long dram_cycles, uint64_t clk, double *total_energy) 
  {
    cur_clk = clk;
    ...
    if (level == T::Level::Rank)
    {
      ...
      *total_energy = power.calculate_energy();
      power.print_energy();
    }
    ...
    double total_child_energy = 0;

    for (auto child : children)
    {
      double child_energy = 0;
      child->finish(dram_cycles, clk, &child_energy);
      total_child_energy += child_energy;
    }

    // if level is not Rank, total_energy is total_child_energy
    if (level != T::Level::Rank)
      *total_energy = total_child_energy;
  }
