...
void DDR5::init_speed(const YAML::Node& speed_config)
{
  ...
  speed_entry[int(TimingCons::nREFI)] = get_nCK_JEDEC_int(REFI_AB_TABLE[density_id] / (1 << int(refresh_mode)), tCK_ps);
  speed_entry[int(TimingCons::nRFCsb)] = get_nCK_JEDEC_int(REFI_AB_FINE_TABLE[density_id] / (1 << int(refresh_mode)) / (org_entry.count[int(Level::Bank)]), tCK_ps);

  // nRFM = rfc for the same mode (e.g., if all banks, then trfc1),   nREFSBRD = refsb
  speed_entry[int(TimingCons::nRFM)] = speed_entry[int(TimingCons::nRFC)];
  speed_entry[int(TimingCons::nREFSBRD)] = speed_entry[int(TimingCons::nRFCsb)];
