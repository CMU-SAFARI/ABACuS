...
DDR4::DDR4(const YAML::Node& config): config(config)
{
  ...
  rega_act_pre_multiplier = config["rega_act_pre_multiplier"].as<int>(1);
...
void DDR4::reload_options(const YAML::Node& config)
{
  rega_act_pre_multiplier = config["rega_act_pre_multiplier"].as<int>(1);

  if (config["spec"])
  {
    if (config["spec"]["speed"])
    {
      const YAML::Node& speed_config = config["spec"]["speed"];
      ...
      for (int i = 1; i < int(TimingCons::MAX); i++)
      {
        ...
        int tCK_ps = speed_entry[int(TimingCons::tCK_ps)];
      ...
      // reset all timing table entries
      for (int i1 = 0 ; i1 < int(Level::MAX); i1++)
        for (int i2 = 0 ; i2 < int(Command::MAX); i2++)
          timing[i1][i2].clear();
...
void DDR4::init_prereq()
{
  ...
  // ACTvrr
  prereq[int(Level::Bank)][int(Command::ACTvrr)] = 
  [] (DRAM<DDR4>* node, Command cmd, int id) 
  {
    switch (int(node->state)) 
    {
      case int(State::ClosedPending):       return Command::ACTvrr;
      case int(State::Closed):              return Command::MAX;
      case int(State::Opened):              return Command::PRE;
      case int(State::OpenedVictimRefresh): return Command::PRE;
      default: assert(false && "[PREQ] Invalid bank state for an ACT command!"); return Command::MAX;
    }
  };
...
void DDR4::init_rowhit()
{
  ...
  [] (DRAM<DDR4>* node, Command cmd, int id) 
  {
    switch (int(node->state)) 
    {
      ...
      case int(State::OpenedVictimRefresh): return false;
      case int(State::Opened):
        if (node->row_state.find(id) != node->row_state.end())
          return true;
        else
...
void DDR4::init_rowopen()
{
    ...
    [] (DRAM<DDR4>* node, Command cmd, int id) 
    {
      ...
      {
        case int(State::Closed): return false;
        case int(State::OpenedVictimRefresh): return true;
...
void DDR4::init_lambda()
{
  ...
  lambda[int(Level::Bank)][int(Command::ACTvrr)] = 
  [] (DRAM<DDR4>* node, int id) 
  {
    node->state = State::OpenedVictimRefresh;
    node->row_state[id] = State::OpenedVictimRefresh;
  };
...
void DDR4::init_power_lambda()
{
  ...
  [this] (DRAM<DDR4>* node) 
  {
    ...
    node->power.handle_ACT_bank(clk, rega_act_pre_multiplier);
  };

  power_lambda[int(Level::Bank)][int(Command::ACTvrr)] = 
  [] (DRAM<DDR4>* node) 
  {
    uint clk =  node->cur_clk;
    node->power.handle_ACT_bank(clk, 1);
  };
  ...
  [this] (DRAM<DDR4>* node) 
  {
    ...
    for (auto bg : node->children)
    {
      for (auto bank : bg->children) 
      {
        if ((bank->state == State::Opened) || (bank->state == State::OpenedVictimRefresh))
          is_rank_idle = false;
      }
    }
    
    uint clk =  node->cur_clk;
    node->power.handle_ACT_rank(is_rank_idle, clk, rega_act_pre_multiplier);
  };

  power_lambda[int(Level::Rank)][int(Command::ACTvrr)] = 
  [] (DRAM<DDR4>* node) 
  {
    bool is_rank_idle = true;
    for (auto bg : node->children)
    ...
    uint clk =  node->cur_clk;
    node->power.handle_ACT_rank(is_rank_idle, clk, 1);
  };
  /************************************************
   *                 PRE Behavior
   ***********************************************/
  ...
  {
    uint clk =  node->cur_clk;
    node->power.handle_PRE_bank(clk, rega_act_pre_multiplier);
  };
  ...
  [this] (DRAM<DDR4>* node) 
  {
    ...
    uint clk =  node->cur_clk;
    node->power.handle_PRE_rank(is_rank_idle, clk, rega_act_pre_multiplier);
  };
  ...
  [this] (DRAM<DDR4>* node) 
  {
    ...
    for (auto bg : node->children)
      for (auto bank : bg->children) 
        bank->power.handle_PRE_bank(bank->cur_clk, rega_act_pre_multiplier);
  };
...
void DDR4::init_timing_table()
{
  ...
  t[int(Command::ACTvrr)]. push_back({Command::ACT,  1, TS(nRRDS)});
  t[int(Command::ACT)]. push_back({Command::ACTvrr,  1, TS(nRRDS)});

  t[int(Command::ACTvrr)]. push_back({Command::ACT,  4, TS(nFAW)});
  t[int(Command::ACT)]. push_back({Command::ACTvrr,  4, TS(nFAW)});

  t[int(Command::ACTvrr)]. push_back({Command::ACTvrr,  1, TS(nRRDS)});
  t[int(Command::ACTvrr)]. push_back({Command::ACTvrr,  4, TS(nFAW)});
  t[int(Command::ACTvrr)]. push_back({Command::PREab, 1, TS(nRAS)});
  t[int(Command::PREab)].push_back({Command::ACTvrr,  1, TS(nRP)});
  ...
  t[int(Command::ACTvrr)]. push_back({Command::REF, 1, TS(nRC)});
  t[int(Command::REF)]. push_back({Command::ACTvrr, 1, TS(nRFC)});
  ...
  t[int(Command::ACTvrr)].push_back({Command::PDE,  1, 1});
  t[int(Command::PDX)].push_back({Command::ACTvrr,  1, TS(nXP)});
  ...
  t[int(Command::SRX)]. push_back({Command::ACTvrr, 1, TS(nXS)});
  ...
  t[int(Command::ACT)].push_back({Command::ACTvrr, 1, TS(nRRDL)});
  t[int(Command::ACTvrr)].push_back({Command::ACT, 1, TS(nRRDL)});
  t[int(Command::ACTvrr)].push_back({Command::ACTvrr, 1, TS(nRRDL)});
  ...
  t[int(Command::ACTvrr)].push_back({Command::ACT, 1, TS(nRC)});
  t[int(Command::ACT)].push_back({Command::ACTvrr, 1, TS(nRC)});

  t[int(Command::ACTvrr)].push_back({Command::ACTvrr, 1, TS(nRC)});
  t[int(Command::ACTvrr)].push_back({Command::PRE, 1, TS(nRAS)});
  t[int(Command::PRE)].push_back({Command::ACTvrr, 1, TS(nRP)});
}
