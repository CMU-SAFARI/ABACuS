...
namespace Ramulator
{
    ...
    id(coreid), llc(llc), processor(processor), trace(trace_fname), memory(memory)
  {
    ...
    window_full_stall_cycles.name("window_full_stall_cycles_core_" + to_string(id))
                            .desc("window full stall cycles")
                            .precision(0)
                            ;
    ...
    memory_access_cycles_avg.name("avg_memory_access_cycles_core_" + to_string(id))
                        .desc("average memory access cycles in memory time domain")
                        .precision(9)
                        ;
  ...
  double Core::calc_ipc()
  {
      ...
      memory_access_cycles_avg = ((double) memory_access_cycles.value()) / retired;
  ...
  void Core::tick()
  {
      ...
      // printf("core %d clk %ld retired %ld reached_limit %d \n", id, clk, retired, reached_limit);
  ...
  {
      window.set_ready(req.addr, ~(l1_blocksz - 1l), req.depart, req.arrive);
  ...
  void Window::init_stat(int core_id)
  {
    top_of_window_latency.name("top_of_window_latency_core" + to_string(core_id))
      .desc("top of window latency")
      .precision(0);
  }
  ...
  long Window::retire()
  {
      ...
      if (!ready_list.at(tail)) return 0;
  ...
  void Window::set_ready(long addr, int mask, long depart, long arrive)
  {
      if (load == 0) return;
      ...
      for (int i = 0; i < load; i++) {
          ...
          if (i == 0) {
            //assert (depart >= top_of_window_since);
            if (depart != -1)
              top_of_window_latency += depart - top_of_window_since;
            // frequency mismatch
          }
      }
  }
