...
namespace Ramulator
{
  ...
  void Cache::evict(std::list<Line>* lines, std::list<Line>::iterator victim) 
  {
    ...
    else 
    {
      ...
      if (dirty) 
      {
        ...
        write_req.unique_id = cachesys->clk;
  ...
  CacheSystem::CacheSystem(const YAML::Node& config, std::function<bool(Request)> send_memory, uint num_cores) :
  send_memory(send_memory), config(config)
  {
    ...
    if (has_l3)
    {
      ...
      mshr_num_each[uint(Cache::Level::L3)] = config["L3"]["mshr"].as<uint>(16) * num_cores;
