...
namespace Ramulator
{
    ...
    function<bool(Request)> send_memory, MemoryBase* memory)
  {
    ...
    if (processor_config["cache"])
      cachesys = std::shared_ptr<CacheSystem>(new CacheSystem(processor_config["cache"], send_memory, tracenum));
  ...
  void Processor::mem_tick() {
    // call all cores' mem tick functions
  ...
  bool Processor::has_reached_limit(bool warmup) {
    if (!warmup && (time_limit != 0))
    {
      if (cpu_time.value() > time_limit)
        return true;
      else
