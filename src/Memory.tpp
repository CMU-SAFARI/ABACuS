...
namespace Ramulator
{
  ...
  spec(ctrls[0]->channel->spec)
  {
    ...
    // if config controller rh defense is Hydra, we will blacklist some pages
    if(config["translation"]["hydra"].as<std::string>("") == "Unfortunately")
    {
      // find how many rows hydra needs to blacklist in a bank
      // two * number of rows / row size
      int hydra_rows_per_bank = (sz[int(T::Level::Row)] * 2) / (sz[int(T::Level::Column)] * 8);
      // print hydras rows per bank
      std::cout << "Hydra rows per bank: " << hydra_rows_per_bank << std::endl;

      // traverse every cache block address and find out where they belong
      for(uint64_t addr = 0; addr < max_address ; addr += 4096)
      //for(uint64_t addr = 0; addr < 4096*1024 ; addr += 4096)
      {
        // for all cache blocks in page
        bool blacklisted_already = false;
        for(uint64_t cb_addr = addr; cb_addr < addr + 4096; cb_addr += 64)
        {
          // translate the address
          std::vector<int> __addr_vec;
          ctrls[0]->addr_mapper->apply(cb_addr, __addr_vec);
          // print cb addr, rank, bank, bank group, row, column
          // std::cout << "cb_addr: " << cb_addr << " rank: " << __addr_vec[1] << " bank group: " << __addr_vec[2] << " bank: " << __addr_vec[3] << " row: " << __addr_vec[4] << " column: " << __addr_vec[5] << std::endl;
          // if the row is within hydra requested rows, blacklist the page
          if((__addr_vec[int(T::Level::Row)] < hydra_rows_per_bank) && !blacklisted_already)
          {
            // add the address to the blacklist
            translation->blacklist_page(addr);
            // print page and row
            std::cout << "Blacklisted physical page: " << addr/4096 << " row: " << __addr_vec[int(T::Level::Row)] << std::endl;
            // break out of the loop
            blacklisted_already = true;
            break;
          }
        }
      }
      //std::cout << "USING FAST BLACKLISTING, THIS IS WRONG" << std::endl; 
    }
